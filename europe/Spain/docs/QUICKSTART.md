# Spain tool — quick-start guide

*A plain-language card for anyone who just wants to use the tool and read the answer.*

**What it is.** A one-page interactive tool for Spain: move a reform or a shock and instantly see **how much better or worse off Spanish households are**, **what it costs the public purse**, and **who gains and loses**. It reads pre-computed results (the EUROMOD tax-benefit model on the EU-SILC 2024 survey) — it does **not** run a live model — and shows money in **2024 euros**. Open `europe/Spain/tools/dial_tool_spain.html` in any web browser; nothing to install.
...
2. **Drag one slider.** Five sliders: **minimum income (IMV)** up to +50%; the **means-tested child allowance** up to +50%; the **IRPF first-bracket rate cut** (the 9.5% state rate, down to −5 percentage points); an **earnings downturn** (0 to −10%); and an **unemployment shock** (0 to +10 percentage points).
...
**Scenarios — compare ready-made reforms.** The baseline plus twelve fixed reforms and shocks (for example IMV +25%, IRPF −2pp, unemployment +5pp), each with its **cost**, its **winners and losers**, its **welfare** effect and its **value-for-money**, plus the **spine** (below).
...
- **The "spine" (headline comparison)** — a minimum-income boost (**IMV +50%**) against a tax cut (**IRPF −1pp**). The two are **not the same size**: their costs are about **12.5%** apart, with the tax cut the cheaper, so comparing their absolute welfare would mostly tell you which reform is larger. The comparison is therefore **per euro of exchequer cost**. On that measure the two are **exactly equal at ε=0** — 1.000 against 1.000, where welfare is simply the money moved — and the minimum-income boost **pulls ahead as ε rises**: **1.601 against 0.785** at ε=1 and 2.590 against 0.709 at ε=2. It is a statement about ratios, not a claim that one reform beats the other outright.
...
- Built on the **EU-SILC data** (2024 wave), run through EUROMOD — not EUROMOD's own bundled input dataset (the EMSD), which prepares its inputs its own way. The welfare and distributional results are validated; how much this input difference moves the fiscal (cost) figures is not known.
