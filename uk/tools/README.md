# The United Kingdom tool

`dial_tool_uk.html` is Deliverable D2.2's tool for the United Kingdom, and it is the whole
thing: one self-contained HTML file. It needs no server, no build step, no installation and
no data. Every figure it shows is already inside it, and it reads nothing from the network.
...
The page carries its own copy of the UK response surface, embedded when the tool was built.
...
That surface was computed with **PolicyEngine UK** over the **Family Resources Survey
2023-24**. Neither the engine nor the microdata is in this repository, and using the tool
needs neither: the surface is the aggregate result.
...
* **Analyst mode** — move one reform dial and read the welfare, distributional, fiscal and
  poverty effects. Seven dials and one on/off toggle, one at a time: moving any dial resets
  the others, because reforms interact and their effects are not additive.
...
* **The provider chains** — [`../../europe/docs/VALUATION_CHAINS.md`](../../europe/docs/VALUATION_CHAINS.md),
  which prints every provider outcome value as `raw x effect share x attribution`. It sits
  under `europe/` but covers all four countries, the United Kingdom included.
